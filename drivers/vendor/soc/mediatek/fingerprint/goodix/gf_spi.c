...
static void gf_report_uevent(struct gf_dev *gf_dev, char *str)
{
	...
	if (gf_dev->pdev) {
		gf_debug(INFO_LOG, "%s gf_dev->pdev is not null!\n", __func__);
		kobject_uevent_env(&(gf_dev->pdev->dev.kobj), KOBJ_CHANGE, envp);
	} else {
		gf_debug(ERR_LOG, "%s gf_dev->pdev is null!\n", __func__);
	}
}
...
static void gf_enable_irq(struct gf_dev *gf_dev)
{
	...
	if (gf_dev->irq_enabled) {
		gf_debug(WARN_LOG, "IRQ has been enabled.\n");
	} else {
...
static void gf_disable_irq(struct gf_dev *gf_dev)
{
	...
	if (gf_dev->irq_enabled) {
		...
		gf_debug(WARN_LOG, "IRQ has been disabled.\n");
	}
}
...
/*	if (gf_dev->irq  > 0){
		gf_debug(WARN_LOG, "the irq(%d) has been registered, so free it\n",  gf_dev->irq);
	...
	if (status) {
		gf_debug(ERR_LOG, "failed to request IRQ:%d\n", gf_dev->irq);
		return status;
	}
...
static int gf_open(struct inode *inode, struct file *filp)
{
	...
	list_for_each_entry(gf_dev, &device_list, device_entry) {
		if (gf_dev->devt == inode->i_rdev) {
			gf_debug(INFO_LOG, "goodixfp_dev Found!\n");
			status = 0;
			break;
		}
	}
	...
	if (dnode) {
		gf_debug(INFO_LOG, "fp-nav device node found!\n");
		gf_dev->pdev = of_find_device_by_node(dnode);
		if (gf_dev->pdev)
		{
			gf_debug(INFO_LOG, "fp-nav device uevent found!\n");
		} else {
	...
	if (status == 0) {
		...
		gf_debug(INFO_LOG, "Succeed to open goodixfp_dev!irq = %d\n",
				gf_dev->irq);
		if (gf_dev->users == 1) {
...
static int gf_release(struct inode *inode, struct file *filp)
{
	...
	gf_dev->users--;
	if (!gf_dev->users) {
		gf_debug(INFO_LOG, "disble_irq. irq = %d\n", gf_dev->irq);
		gf_disable_irq(gf_dev);
		gf_debug(DEBUG_LOG, "%s : free_irq\n", __func__);
		free_irq(gf_dev->irq,  gf_dev);
		gf_cleanup(gf_dev);
		...
		unsigned long val, void *data)
{
	...
	if (val != FB_EARLY_EVENT_BLANK)
		return 0;
	gf_debug(INFO_LOG, "[info] %s go to the goodix_fb_state_chg_callback value = %d\n",
			__func__, (int)val);
...
//#endif
...
#ifdef AP_CONTROL_CLK
	gf_debug(INFO_LOG, "Get the clk resource.\n");
	/* Enable spi clock */
	...
	gf_debug(INFO_LOG, "version : %d .%d .%02d\n", VER_MAJOR, VER_MINOR, PATCH_LEVEL);
...
static int __init gf_init(void)
{
	...
	gf_debug(INFO_LOG, "gf platform_driver_register ok ---status : %d\n", status);
#elif defined(USE_SPI_BUS)
	status = spi_register_driver(&gf_driver);
	gf_debug(INFO_LOG, "gf spi_register_driver ok ---status : %d\n", status);
#endif
	if (status < 0) {
