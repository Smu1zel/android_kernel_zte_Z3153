...
static int sf_probe(sf_device_t *dev)
{
...
#endif
    xprintk(KERN_INFO, "Start register input device\n");
    ...
    xprintk(KERN_INFO, "Start register misc device\n");
    ...
    xprintk(KERN_INFO, "Start register sysfs sf_attribute_group\n");
...
#else
    xprintk(KERN_INFO, "Start register sf_fb_notifier_callback\n");
...
static int sf_read_sensor_id(void)
{
    ...
    do {
        ...
        if ((0x46 == readbuf[7]) && (0x74 == readbuf[8]) && (0x53 == readbuf[9])
            && (0x73 == readbuf[10]) && (0x8e == readbuf[11])) {
