...
void himax_release_all_event(struct himax_ts_data *ts)
{
	int loop_i = 0;
...
static int tpd_init_tpinfo(struct tpd_classdev_t *cdev)
{
	...
	if (ts->suspended) {
		E("tp suspend, get tp info error");
		return -EIO;
	}
	I("tpd_init_tpinfo\n");
...
static int tpd_get_singletapgesture(struct tpd_classdev_t *cdev)
{
	...
	cdev->b_single_tap_enable = ts->gesture_cust_en[1];
	return cdev->b_single_tap_enable;
}
...
static int tpd_set_singletapgesture(struct tpd_classdev_t *cdev, int enable)
{
	...
	enable = enable ? 1 : 0;
	ts->SMWP_enable = ts->gesture_cust_en[0] || enable;
	if (enable) {
		ts->gesture_cust_en[1] = 1;
		ts->gesture_cust_en[2] = 1;
		ts->gesture_cust_en[3] = 1;
	} else {
	...
	UFP_INFO("%s: SMART_WAKEUP_enable = %d.\n", __func__, HX_SMWP_EN);
...
static int tpd_get_wakegesture(struct tpd_classdev_t *cdev)
{
	...
	UFP_INFO("%s wakeup_gesture_enable val is:%d.\n", __func__, ts->SMWP_enable);
	cdev->b_gesture_enable = ts->gesture_cust_en[0];
...
static int tpd_enable_wakegesture(struct tpd_classdev_t *cdev, int enable)
{
	...
	enable = enable ? 1 : 0;
	ts->SMWP_enable = ts->gesture_cust_en[1] || enable;
	ts->gesture_cust_en[0] = enable;
	if (!ts->suspended) {
		g_core_fp.fp_set_SMWP_enable(ts->SMWP_enable, ts->suspended);
	} else {
		cdev->tp_suspend_write_gesture = true;
...
static int tpd_set_one_key(struct tpd_classdev_t *cdev, int enable)
{
	...
	enable = enable ? 1 : 0;
	ts->open_one_key = enable;
	UFP_INFO("%s: tpd set one key  = %d.\n", __func__, ts->open_one_key);
	return 0;
}
...
static int tpd_get_one_key(struct tpd_classdev_t *cdev)
{
	...
	cdev->one_key_enable = ts->open_one_key;
...
#ifdef CONFIG_TPD_UFP_MAC
int himax_tp_resume(void *ts_data)
{
	struct himax_ts_data *ts = (struct himax_ts_data *)ts_data;

	himax_chip_common_resume(ts);
	return 0;
}

int himax_tp_suspend(void *ts_data)
{
	struct himax_ts_data *ts = (struct himax_ts_data *)ts_data;
...
void himax_tpd_register_fw_class(void)
{
	...
	ufp_tp_ops.tp_data = private_ts;
	ufp_tp_ops.tp_resume_func = himax_tp_resume;
	ufp_tp_ops.tp_suspend_func = himax_tp_suspend;
#endif
}
