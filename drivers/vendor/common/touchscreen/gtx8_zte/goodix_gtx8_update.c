...
extern int goodix_parse_cfg_bin(struct goodix_cfg_bin *cfg_bin);
extern int goodix_get_reg_and_cfg(struct goodix_ts_device *ts_dev, struct goodix_cfg_bin *cfg_bin);
...
const struct ts_firmware *gtp_tp_requeset_firmware(char *file_name)
{
	...
	if (IS_ERR(file)) {
		ts_err("open %s file fail, try open /vendor/firmware/", file_path);
		snprintf(file_path, sizeof(file_path), "%s%s", "/vendor/firmware/", file_name);
		file = filp_open(file_path, O_RDONLY, 0);
		if (IS_ERR(file)) {
			ts_err("open %s file fail", file_path);
			return NULL;
		}
	}
	...
	ts_info("open %s file ,firmware->size:%d", file_path, firmware->size);
	firmware->data = vmalloc(firmware->size);
	if (firmware->data == NULL) {
		ts_err("alloc firmware data failed");
	...
	if (ret < 0) {
		ts_err("Request from fs read whole file failed %d", ret);
		goto err_free_firmware_data;
	}
	ts_info("Request from fs read whole file successful");
...
static int gtp_cfg_bin_upgrade(struct goodix_ts_core *core_data, const struct ts_firmware *gtp_firmware)
{
	...
	ret = goodix_get_reg_and_cfg(ts_dev, cfg_bin);
	if (!ret) {
	...
	ret = ts_dev->hw_ops->send_config(ts_dev, &(ts_dev->normal_cfg));
	if (ret < 0) {
...
int gtp_tp_fw_upgrade(struct tpd_classdev_t *cdev, char *fw_name, int fwname_len)
{
	...
	gtp_firmware = gtp_tp_requeset_firmware(fileName);
