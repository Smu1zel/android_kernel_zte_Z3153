...
static void fts_psensor_enable(struct fts_ts_data *data, int enable)
{
	...
	FTS_INFO("%s is %d", __func__, enable);
	if (data->client == NULL)
		return;
	...
	data->psensor_pdata->tp_psensor_opened = enable;
}
				  ...
				  unsigned int enable)
{
	...
	psensor_pdata->tp_psensor_enable = enable;
	if (data->suspended == true) {
		FTS_INFO("%s:tp is suspended,can not set psensor %d\n", __func__, enable);
...
int fts_sensor_resume(struct fts_ts_data *data)
{
	...
	if (data->psensor_pdata->tp_psensor_enable) {
...
{
	return snprintf(buf, 64, "%d\n", fts_psensor_pdata->tp_psensor_enable);
}
					...
					size_t count)
{
	...
	if (flush_count % 2 == 0) {
		input_report_abs(fts_psensor_pdata->input_psensor_dev, ABS_DISTANCE, -1);
		flush_count = 1;
	} else {
		input_report_abs(fts_psensor_pdata->input_psensor_dev, ABS_DISTANCE, -2);
		flush_count = 0;
	}

	input_mt_sync(fts_psensor_pdata->input_psensor_dev);
	input_sync(fts_psensor_pdata->input_psensor_dev);
...
int fts_sensor_init(struct fts_ts_data *data)
{
	...
	input_set_abs_params(psensor_input_dev, ABS_DISTANCE, -2, 1, 0, 0);
