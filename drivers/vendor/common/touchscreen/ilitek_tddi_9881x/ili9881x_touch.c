...
static void dma_clear_reg_setting(void)
{
	...
	if (ili_ice_mode_bit_mask_write(DMA50_ADDR,
		DMA50_reg_dma_ch0_src1_format | DMA50_reg_dma_ch0_src1_en, BIT(31)) < 0)
...
static void dma_trigger_reg_setting(u32 reg_dest_addr, u32 flash_start_addr, u32 copy_size)
{
	...
	if (ili_ice_mode_write(DMA54_ADDR, DMA54_reg_dma_ch0_dest_format
		| DMA54_reg_dma_ch0_dest_en, BIT(31)) < 0)
	...
	if (ili_ice_mode_bit_mask_write(DMA55_ADDR,
		DMA55_reg_dma_ch0_trafer_mode, (0 << 24)) < 0)
...
void ili_report_gesture_mode(u8 *buf, int len)
{
	...
	len = (len <= P5_X_GESTURE_INFO_LENGTH) ?  len : P5_X_GESTURE_INFO_LENGTH;
	for (i = 0; i < len; i++)
		ges[i] = buf[i];
