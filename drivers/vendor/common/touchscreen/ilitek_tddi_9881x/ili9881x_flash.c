...
static int ilitek_fw_check_ddi_chunk(u8 *pfw)
{
	...
	cmd[0] = CMD_GET_FLASH_DATA;
	cmd[1] = DDI_RSV_BK_SIZE & 0xFF;
	cmd[2] = (DDI_RSV_BK_SIZE >> 8) & 0xFF;
	...
	for (i = 0; i < (FW_BLOCK_INFO_NUM - 1); i++) {
		...
		if (((blk_map >> i) & 0x01) == 0x01) {
			fbi[num].start = (CTPM_FW[132 + i * 6] << 16)
				| (CTPM_FW[133 + i * 6] << 8) | CTPM_FW[134 + i * 6];
			fbi[num].end = (CTPM_FW[135 + i * 6] << 16)
				| (CTPM_FW[136 + i * 6] << 8) |  CTPM_FW[137 + i * 6];
