...
#ifdef ZTE_RESOLVE_ADAPTOR_QC_SWITCH_MTK
bool zte_mtk_pe_whether_is_7v_to_9v_step = false;
void zte_forcelly_disable_mtk_hv_charging(void)
{
	if (pinfo && pinfo->chg1_dev) {
		pinfo->enable_hv_charging = false;
		pr_info("%s: forcelly disable hv charging function\n", __func__);
	}
}
#endif
...
/*ZTE add for dual abcct cooler*/
#define NO_CHARGING_LIMIT -1
#define COOLER_1ST 0
#define COOLER_2ND 1

static int charging_limit_1st = -1;
static int charging_limit_2nd = -1;

int charger_manager_set_dual_charging_current_limit(
		struct charger_consumer *consumer, int idx, int charging_current)
{
	struct charger_manager *info = consumer->cm;

	if (info != NULL) {
		struct charger_data *pdata_main;
		struct charger_data *pdata_slave;
		int charging_current_set = -1;

		pdata_main = &info->chg1_data;
		pdata_slave = &info->chg2_data;

		if (idx == COOLER_1ST)
			charging_limit_1st = charging_current;
		else if (idx == COOLER_2ND)
			charging_limit_2nd = charging_current;
		else
			return -ENOTSUPP;
		if (charging_limit_1st == NO_CHARGING_LIMIT) {
			charging_current_set = charging_limit_2nd;
			chr_err("%s step1", __func__);
		} else if (charging_limit_2nd == NO_CHARGING_LIMIT) {
			charging_current_set = charging_limit_1st;
			chr_err("%s step2", __func__);
		} else {
			charging_current_set = min(charging_limit_1st, charging_limit_2nd);
			chr_err("%s step3", __func__);
		}
		chr_err("%s: setting_limit:%d, 1st_limit:%d, 2nd_limit:%d, final_limit:%d\n",
				__func__,
				charging_current,
				charging_limit_1st,
				charging_limit_2nd,
				charging_current_set
			);

		pdata_main->thermal_charging_current_limit = charging_current_set;
		pdata_slave->thermal_charging_current_limit = charging_current_set;
		chr_err("%s: dev:%s idx:dual cooler_idx:%d en:%d\n", __func__,
			dev_name(consumer->dev), idx, charging_current_set);
		if (info->chr_type != CHARGER_UNKNOWN) {
			_mtk_charger_change_current_setting(info);
			_wake_up_charger(info);
		}
...
/*ZTE add for dual abcct cooler end*/
...
bool sw_jeita_whether_is_normal_step2 = false;
...
void do_sw_jeita_state_machine(struct charger_manager *info)
{
	...
	if (sw_jeita->sm == TEMP_ABOVE_T4) {
		...
		if (battery_get_bat_voltage() * 1000 >= info->data.jeita_temp_t2_to_t3_cv_1 + 100000 ||
			sw_jeita_whether_is_normal_step2 == true) {
...
#define ZTE_ADAPTOR_QC_TO_MTK_MSECOND 1000
void mtk_charger_int_handler(void)
{
	...
	if (mt_get_charger_type() == CHARGER_UNKNOWN) {
		...
		charger_manager_notifier(pinfo, CHARGER_NOTIFY_START_CHARGING);
#ifdef ZTE_RESOLVE_ADAPTOR_QC_SWITCH_MTK
		if (zte_mtk_pe_whether_is_7v_to_9v_step &&
			((delta_time.tv_sec * MSEC_PER_SEC + delta_time.tv_nsec / NSEC_PER_MSEC) <= ZTE_ADAPTOR_QC_TO_MTK_MSECOND))
			zte_forcelly_disable_mtk_hv_charging();
#endif
	}
				...
				enum charger_type chr_type)
{
	...
	sw_jeita_whether_is_normal_step2 = false;
	...
	if (info->chr_type == STANDARD_CHARGER &&
		battery_get_soc() >= info->data.ta_start_battery_soc &&
		battery_get_soc() < info->data.ta_stop_battery_soc)
...
static ssize_t show_cc_tuning_dual_polling_ieoc(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct charger_manager *info = dev->driver_data;

	pr_debug("dual_polling_ieoc : %d\n", info->data.dual_polling_ieoc);
	return snprintf(buf, 10, "%d\n", info->data.dual_polling_ieoc);
}

static ssize_t store_cc_tuning_dual_polling_ieoc(struct device *dev, struct device_attribute *attr, const char *buf,
				 size_t size)
{
	struct charger_manager *info = dev->driver_data;
	int reg = 0;
	...
	if (info != NULL && buf != NULL && size != 0) {
		if (kstrtoint(buf, 10, &reg) == 0) {
			info->data.dual_polling_ieoc = reg;
			_wake_up_charger(info);
			pr_debug("%s: %d\n", __func__, info->data.dual_polling_ieoc);
		} else {
...
static DEVICE_ATTR(cc_tuning_dual_polling_ieoc, 0664, show_cc_tuning_dual_polling_ieoc, store_cc_tuning_dual_polling_ieoc);
...
static void charger_update_data(struct charger_manager *info)
{
	...
	if (force_update_batt_temp != 25)
		info->battery_temp = force_update_batt_temp;
}
...
bool zte_thermal_whether_is_limiting(struct charger_manager *info)
{
...
static int charger_routine_thread(void *arg)
{
	...
	int bat_voltage = 0, chg_voltage = 0;
	uint chg_status = 0;
	int chg_efficiency = 100;
	...
	while (1) {
		...
		chg_current = charger_get_ibus();
		chg_status = get_bat_status();
		bat_voltage = battery_get_bat_voltage();
		chg_voltage = charger_get_vbus();;
		charger_update_data(info);

		chr_err("Vbat=%d,Vbus=%d,Ibat=%d,Ibus=%d,Status=%s,VChr=%d,T=%d,Soc=%d:%d,CT:%d:%d hv:%d pd:%d:%d\n",
			bat_voltage, chg_voltage, bat_current, chg_current, battery_chg_status_name[chg_status],
		...
		if (is_disable_charger() == false) {
			if (is_charger_on == true) {
				if (chg_voltage > 0 && chg_current > 0) {
					chg_efficiency = 100 * (bat_voltage * bat_current) / (chg_voltage * chg_current);
					chr_info("charger IC efficiency is %d\n", chg_efficiency);
				}
				...
				struct device *dev)
{
	...
	if (of_property_read_u32(np, "jeita_temp_t2_to_t3_cv_1", &val) >= 0) {
		info->data.jeita_temp_t2_to_t3_cv_1 = val;
	} else {
		info->data.jeita_temp_t2_to_t3_cv_1 = info->data.jeita_temp_t2_to_t3_cv;
		chr_err("use default jeita_temp_t2_to_t3_cv_1:%d\n",
			info->data.jeita_temp_t2_to_t3_cv_1);
	}
	...
	info->data.ta_7v_support = of_property_read_bool(np, "ta_7v_support");
	info->data.ta_pe20_step_adjust_support = of_property_read_bool(np, "ta_pe20_step_adjust_support");

	pr_info("ta_12v_support: %d, ta_9v_support: %d, ta_7v_support: %d\n",
		info->data.ta_12v_support, 	info->data.ta_9v_support, info->data.ta_7v_support);
	...
	if (of_property_read_u32(np, "chg1_ta_ac_charger_current_1", &val) >= 0)
		info->data.chg1_ta_ac_charger_current_1 = val;
	else {
		chr_err("use default chg1_ta_ac_charger_current:%d\n",
			info->data.chg1_ta_ac_charger_current);
		info->data.chg1_ta_ac_charger_current_1 =
						info->data.chg1_ta_ac_charger_current;
	}

	if (of_property_read_u32(np, "chg2_ta_ac_charger_current_1", &val) >= 0)
		info->data.chg2_ta_ac_charger_current_1 = val;
	else {
		chr_err("use default chg2_ta_ac_charger_current:%d\n",
			info->data.chg2_ta_ac_charger_current);
		info->data.chg2_ta_ac_charger_current_1 =
						info->data.chg2_ta_ac_charger_current;
	}
...
static int mtk_charger_setup_files(struct platform_device *pdev)
{
	...
	ret = device_create_file(&(pdev->dev), &dev_attr_cc_tuning_dual_polling_ieoc);
	if (ret)
		goto _out;
...
static int mtk_charger_probe(struct platform_device *pdev)
{
	...
	info->chg2_data.input_current_limit = 500000;
