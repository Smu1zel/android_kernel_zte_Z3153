...
{
#ifdef CONFIG_VENDOR_POWER_VOTER
	if (!strcmp(dev_name(&chg_dev->dev), PRIMARY_CHARGER_DEVICE_NMAE)) {
		if (get_client_vote(charge_votable_data.battery_charging_enabled_votable, MTK_SETTING_VOTER) != en)
			vote(charge_votable_data.battery_charging_enabled_votable, MTK_SETTING_VOTER, true, en);
		else
			rerun_election(charge_votable_data.battery_charging_enabled_votable);
		return 0;
	} else {
...
int charger_dev_send_ta_current_pattern_zte(struct charger_device *chg_dev,
					bool is_increase)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->send_ta_current_pattern_zte)
		return chg_dev->ops->send_ta_current_pattern_zte(chg_dev,
							     is_increase);
...
int charger_dev_send_ta20_current_pattern_zte(struct charger_device *chg_dev,
					  u32 uV)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->send_ta20_current_pattern_zte)
		return chg_dev->ops->send_ta20_current_pattern_zte(chg_dev, uV);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_send_ta20_current_pattern_zte);
...
static void __exit charger_class_exit(void)
{
	...
	destroy_votable(charge_votable_data.battery_charging_enabled_votable);
#endif
			...
			void *data, int en, const char *client)
{
	...
	if (charger_dev != NULL && charger_dev->ops != NULL && charger_dev->ops->enable)
		charger_dev->ops->enable(charger_dev, en);
				...
				union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		pval->intval = get_client_vote(pdata->usb_icl_votable, POLICY_SETTING_VOTER);
		pval->intval = (pval->intval < 0) ? 1 : 0;
		break;
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		pval->intval = get_client_vote(pdata->battery_charging_enabled_votable, POLICY_SETTING_VOTER);
		pval->intval = (pval->intval < 0) ? 1 : 0;
		break;
	default:
				...
				const union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_RECHARGE_SOC:
		...
		break;
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		vote(pdata->usb_icl_votable, POLICY_SETTING_VOTER,
								(pval->intval ? false : true), 0);
		break;
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		vote(pdata->battery_charging_enabled_votable, POLICY_SETTING_VOTER,
								(pval->intval ? false : true), 0);
		break;
	default:
		cas_debug("interface unsupported property %d\n", psp);
...
*/
	...
	charge_votable_data.battery_charging_enabled_votable = create_votable("CHARGING_ENABLE", VOTE_MIN,
					charger_battery_charging_enabled_vote_callback,
					&charge_votable_data);
	if (IS_ERR(charge_votable_data.battery_charging_enabled_votable)) {
		rc = PTR_ERR(charge_votable_data.battery_charging_enabled_votable);
