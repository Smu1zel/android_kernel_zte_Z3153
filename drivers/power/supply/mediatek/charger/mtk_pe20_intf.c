...
static int __pe20_set_ta_vchr_zte(struct charger_manager *pinfo, u32 chr_volt)
{
	int ret = 0;
	struct mtk_pe20 *pe20 = &pinfo->pe2;
	bool chg2_chip_enabled = false;
	...
	ret = charger_dev_send_ta20_current_pattern_zte(pinfo->chg1_dev, chr_volt);
	if (ret < 0) {
		chr_err("%s: failed, ret = %d\n", __func__, ret);
		return ret;
	}
...
static int pe20_set_ta_vchr(struct charger_manager *pinfo, u32 chr_volt)
{
	...
	do {
		...
		if (retry_cnt < 2)
			...
			ret = __pe20_set_ta_vchr_zte(pinfo, chr_volt);
...
static int pe20_init_ta(struct charger_manager *pinfo)
{
	...
	pe20->to_first_tune_ta_vchr = true;
...
int mtk_pe20_check_charger(struct charger_manager *pinfo)
{
	...
	if (pinfo->enable_pe_2 != true || charger_policy_get_status() == true)
		return -ENOTSUPP;
...
int mtk_pe20_start_algorithm(struct charger_manager *pinfo)
{
...
#ifdef ZTE_CHARGER_NOTIFY_MKT_FAST_CHARGER
	} else if (battery_get_soc() < pinfo->data.ta_start_battery_soc ||
		battery_get_soc() >= pinfo->data.ta_stop_battery_soc) {
		ret = pe20_leave(pinfo);
		goto out;
#endif
	...
	pe20->to_first_tune_ta_vchr = false;

	if (pinfo->data.ta_pe20_step_adjust_support == true) {
		...
		pe20_set_mivr(pinfo, vbus - 1000000);
	}
...
void mtk_pe20_set_is_plug_in(struct charger_manager *pinfo)
{
	mutex_lock(&pinfo->pe2.pmic_sync_lock);
	pinfo->pe2.is_connect = false;
	pinfo->pe2.is_enabled = true;
	mutex_unlock(&pinfo->pe2.pmic_sync_lock);
}

void mtk_pe20_set_is_plug_out(struct charger_manager *pinfo)
{
	mutex_lock(&pinfo->pe2.pmic_sync_lock);
	pinfo->pe2.is_connect = false;
	pinfo->pe2.is_enabled = true;
...
int mtk_pe20_init(struct charger_manager *pinfo)
{
	...
	pinfo->pe2.is_enabled = true;
	pinfo->pe2.to_first_tune_ta_vchr = true;
