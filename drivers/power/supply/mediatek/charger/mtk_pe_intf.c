...
static int pe_set_mivr(struct charger_manager *pinfo, int uV)
{
	...
	if (uV <= 0) {
		chr_err("%s: mivr values is negative, uv = %d\n", __func__, uV);
		return ret;
	}
...
static int __pe_increase_ta_vchr_zte(struct charger_manager *pinfo)
{
	...
	if (mt_get_charger_type() != CHARGER_UNKNOWN) {
		ret = charger_dev_send_ta_current_pattern_zte(pinfo->chg1_dev,
							increase);
...
static int pe_increase_ta_vchr(struct charger_manager *pinfo, u32 vchr_target)
{
	...
	do {
		...
		retry_cnt++;
	} while (mt_get_charger_type() != CHARGER_UNKNOWN && retry_cnt < 4 &&
...
static int pe_detect_ta(struct charger_manager *pinfo)
{
	...
	if (ret == 0) {
		...
		zte_mtk_pe_whether_is_7v_to_9v_step = true;
#endif
...
static int pe_init_ta(struct charger_manager *pinfo)
{
	...
	chr_debug("%s: starts\n", __func__);
	if (pinfo->data.ta_7v_support || pinfo->data.ta_9v_support ||
		pinfo->data.ta_12v_support)
...
int mtk_pe_start_algorithm(struct charger_manager *pinfo)
{
	...
	if (!pe->to_tune_ta_vchr) {
		...
		goto _out;
#ifdef ZTE_CHARGER_NOTIFY_MKT_FAST_CHARGER
	} else if (pinfo->data.ta_start_battery_soc > battery_get_soc() ||
			pinfo->data.ta_stop_battery_soc <= battery_get_soc()) {
		chr_info("%s: OK, SOC (%d) is out of (%d, %d), stop PE+ when firstly plug in chaptor\n", __func__,
...
#ifdef ZTE_RESOLVE_ADAPTOR_QC_SWITCH_MTK
			zte_mtk_pe_whether_is_7v_to_9v_step = false;
#endif
