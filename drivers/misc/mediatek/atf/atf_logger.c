...
static void atf_time_sync_resume(void)
{
	...
	time_to_sync = ((u64)ts.tv_sec) * NSEC_PER_SEC + ts.tv_nsec;
	pr_info("%s:zte_time_to_sync=%llx x1=0x%x x2=0x%x\n", __func__, time_to_sync,
		(u32)time_to_sync, (u32)(time_to_sync >> 32));
	arm_smccc_smc(MTK_SIP_KERNEL_TIME_SYNC,
		(u32)time_to_sync, (u32)(time_to_sync >> 32), 0, 0,
		0, 0, 0, &res);
}
void zte_atf_time_sync(void)
{
	atf_time_sync_resume();
}
#else
static void atf_time_sync_resume(void)
{
