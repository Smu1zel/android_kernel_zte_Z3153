...
int esd_check_init = 0;/*indicate the resume/suspend state for esd check*/
void primary_display_set_esd_check_flag(int ecd_check_flag)
{
	esd_check_init = ecd_check_flag;
}
...
#endif
	static bool boot_esd_check_delay = true;

	DISPFUNC();
	sched_setscheduler(current, SCHED_RR, &param);

	while (1) {
		if (boot_esd_check_delay) {
			msleep(30000); /* 30s */
			boot_esd_check_delay = false;
		}
		msleep(2000); /* 2s */
		DISPCHECK("[ESD]esd_check_init %d esd_check_enable %d\n", esd_check_init, esd_check_enable);
		if(esd_check_init < 0) {
			continue;
		} else if(esd_check_init < 3) { /*skip 3 times after resume*/
			esd_check_init ++;
			continue;
		}
		...
		do {
			...
			error_cnt ++;
			DISPERR(
				"[ESD]esd check fail, will do esd recovery. try=%d error_cnt %d\n",
				i, error_cnt);
			if (error_cnt > 3)
