...
#include "gc8034z6250cc137mipi_Sensor.h"
		...
		for (i = 0; i < 5; i++) {
			...
			for (j = 0; j < 2; j++)
				if (((temp >> (4 * j + 3)) & 0x01) == 0x01) {
					gc8034_otp_info.reg_page[gc8034_otp_info.reg_num] =
						(temp >> (4 * j)) & 0x03;
		...
		for (i = 0; i < gc8034_otp_info.dd_cnt; i++) {
			...
			case 0:
				gc8034_otp_info.dd_param[i].x =
					DD_WIDTH - gc8034_otp_info.dd_param[i].x + 1;
				break;
			case 1:
				gc8034_otp_info.dd_param[i].x =
					DD_WIDTH - gc8034_otp_info.dd_param[i].x - 1;
			...
			default:
				gc8034_otp_info.dd_param[i].x =
					DD_WIDTH - gc8034_otp_info.dd_param[i].x;
				break;
			}
#endif
#if GC8034_MIRROR_FLIP_ENABLE
			gc8034_otp_info.dd_param[i].y =
				DD_HEIGHT - gc8034_otp_info. dd_param[i].y + 1;
#endif
		}
			...
			for (j = i + 1; j < gc8034_otp_info.dd_cnt; j++) {
				...
				+ gc8034_otp_info.dd_param[j].x) {
					...
					gc8034_otp_info.dd_param[i].x =
						gc8034_otp_info.dd_param[j].x;
					gc8034_otp_info.dd_param[i].y =
						gc8034_otp_info.dd_param[j].y;
					gc8034_otp_info.dd_param[i].t =
						gc8034_otp_info.dd_param[j].t;
...
static void gc8034_otp_identify(void)
{
	...
	gc8034_read_otp_group(0, GC8034_OTP_ID_DATA_OFFSET,
		&gc8034_otp_info.otp_id[0], GC8034_OTP_ID_SIZE);
...
UINT32 GC8034Z6250CC137_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
