...
#include "sp5508z5157v_mipi_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "SP5508Z5157V_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = SP5508Z5157V_SENSOR_ID,

    .checksum_value = 0x8e8fb430,        //checksum value for Camera Auto Test
...
static void write_shutter(kal_uint16 shutter)
{
	...
	// Update Shutter
    shutter = (shutter >> 1) << 1;	//Set shutter as even num
	write_cmos_sensor(0xfd, 0x01);
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				LOG_INF("sp5508_front i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			LOG_INF("sp5508_front Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
LOG_INF("SENSOR_FEATURE_GET_MIPI_PIXEL_RATE\n");
		...
		*(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = rate;
		LOG_INF("SENSOR_FEATURE_GET_MIPI_PIXEL_RATE123 %d\n", rate);
		}
	}
	break;
	...
	case SENSOR_FEATURE_GET_CROP_INFO:
		LOG_INF("SENSOR_FEATURE_GET_CROP_INFO123 scenarioId:%d\n", (UINT32)*feature_data);
...
UINT32 SP5508Z5157V_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}    /*    SP5508MIPI_RAW_SensorInit    */
