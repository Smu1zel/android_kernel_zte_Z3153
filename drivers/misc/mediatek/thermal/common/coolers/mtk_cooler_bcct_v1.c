...
struct charger_consumer *consumer, int idx, int charging_current_uA)
{
	...
	int __attribute__ ((weak))
charger_manager_set_dual_charging_current_limit(
struct charger_consumer *consumer, int idx, int charging_current_uA)
{
	pr_notice("E_WF: %s doesn't exist\n", __func__);
	return 0;
...
#ifdef CONFIG_ZTE_THERMAL_CHARGER_LIMIT_DUAL
		charger_manager_set_dual_charging_current_limit(pthermal_consumer, 0,
				((chrlmt_bat_chr_curr_limit != -1) ?
				chrlmt_bat_chr_curr_limit * 1000 : -1));
#else
