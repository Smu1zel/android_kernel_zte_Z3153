...
static void cfp_report_uevent(struct cdfinger_data *cdfinger, char *str)
{
	...
	if (cdfinger->pdev) {
		cfp_debug(INFO_LOG, "%s cdfinger->pdev is not null!\n", __func__);
		kobject_uevent_env(&(cdfinger->pdev->dev.kobj), KOBJ_CHANGE, envp);
	} else {
		cfp_debug(ERR_LOG, "%s cdfinger->pdev is null!\n", __func__);
	}
}
...
static int cdfinger_reset(int delay_ms)
{
	...
	if (gpio_is_valid(cdfinger->reset_gpio)) {
		...
		msleep(delay_ms);
		cfp_debug(INFO_LOG, "----cdfinger hw reset delay_ms : %d----\n", delay_ms);
	}
	return 0;
}
...
static int cdfinger_open(struct inode *inode, struct file *file)
{
	...
	if (dnode) {
		cfp_debug(INFO_LOG, "fp-nav device node found!\n");
		g_cdfinger->pdev = of_find_device_by_node(dnode);
		if (g_cdfinger->pdev)
		{
