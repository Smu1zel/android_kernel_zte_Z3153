...
static void sil_report_uevent(struct silfp_data *fp_dev, char *str)
{
	...
	if (fp_dev->pdev) {
		LOG_MSG_DEBUG(I_LOG, "%s fp_dev->pdev is not null!\n", __func__);
		kobject_uevent_env(&(fp_dev->pdev->dev.kobj), KOBJ_CHANGE, envp);
	} else {
		LOG_MSG_DEBUG(E_LOG, "%s fp_dev->pdev is null!\n", __func__);
	}
}
...
static int silfp_keyevent(struct silfp_data *fp_dev, struct fp_dev_key_t *pkey)
{
	...
	if (IS_KEY_VALID(pkey->value)) {
		...
		case NAV_KEY_RIGHT:
			...
			LOG_MSG_DEBUG(D_LOG, "%s nav right\n", __func__);
		break;

		case NAV_KEY_LEFT:
			/*sil_report_uevent(fp_dev, FP_NAV_LEFT);*/
			LOG_MSG_DEBUG(D_LOG, "%s nav left\n", __func__);
		break;
...
static int silfp_open(struct inode *inode, struct file *filp)
{
	...
	if (dnode) {
		...
		fp_dev->pdev = of_find_device_by_node(dnode);
		if (fp_dev->pdev)
		{
