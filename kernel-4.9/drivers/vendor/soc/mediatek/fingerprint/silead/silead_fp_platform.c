...
int silfp_fingerprint_power_init(struct silfp_data *fp_dev)
{
	int ret = 0;
	struct device *dev = NULL;

	LOG_MSG_DEBUG(D_LOG, "%s, get regulator from dts\n", __func__);

	if (fp_dev == NULL)  {
		LOG_MSG_DEBUG(E_LOG, "%s, fp_dev is NULL\n", __func__);
		return -ENODEV;
	}

	dev = &fp_dev->spi->dev;

	fp_dev->fp_reg = regulator_get(dev, "vfp");

	if (IS_ERR(fp_dev->fp_reg)) {
		LOG_MSG_DEBUG(E_LOG, "%s get regulator failed\n", __func__);
		return IS_ERR(fp_dev->fp_reg);
	} else {
		LOG_MSG_DEBUG(I_LOG, "%s get regulator success\n", __func__);
	}

	ret = regulator_set_voltage(fp_dev->fp_reg, 2800000, 2800000);
	if (ret) {
		LOG_MSG_DEBUG(E_LOG, "%s regulator_set_voltage(%d) failed\n",
			__func__, ret);
		goto err;
	} else {
		LOG_MSG_DEBUG(I_LOG, "%s regulator_set_voltage(%d) success\n",
			__func__, ret);

	}
	/*ret = regulator_enable(fp_dev->fp_reg);
	if (ret) {
		LOG_MSG_DEBUG(E_LOG, "%s regulator enable failed(%d)\n",
			__func__, ret);
		goto err;
	}*/
...
int silfp_parse_dts(struct silfp_data *fp_dev)
{
	...
	of_property_read_u32(np, "power-type", &fp_dev->power_type);
	LOG_MSG_DEBUG(I_LOG, "%s, get power type[%d] from dts\n",
			__func__, fp_dev->power_type);

	if (fp_dev->power_type == 1) {
		/*  powered by pmic regulator */
		ret = silfp_fingerprint_power_init(fp_dev);
		if (ret) {
			LOG_MSG_DEBUG(E_LOG, "%s, fingerprint_power_init failed.\n", __func__);
			goto err_avdd;
		} else {
			LOG_MSG_DEBUG(I_LOG, "%s, silfp_fingerprint_power_init success.\n", __func__);
		}
	} else {
		/*---powered by gpio control---*/
		fp_dev->avdd_port = of_get_named_gpio(np, "avdd-gpios", 0);
	...
	/*---int port---*/
	fp_dev->int_port = of_get_named_gpio(np, "irq-gpios", 0);
	...
	/*---rst port---*/
	fp_dev->rst_port = of_get_named_gpio(np, "rst-gpios", 0);
...
err_avdd:
	...
	if (fp_dev->power_type == 1) {
		LOG_MSG_DEBUG(E_LOG, "%s, silfp_fingerprint_power_init failed.\n", __func__);
	} else {
		fp_dev->avdd_port = 0;
	}
	return status;
...
int silfp_cleanup(struct silfp_data *fp_dev)
{
       int ret = 0;
	LOG_MSG_DEBUG(D_LOG, "[%s] enter.\n", __func__);
	...
	if (fp_dev->power_type == 1) {
		...
		if (fp_dev->fp_reg != NULL){
			regulator_put(fp_dev->fp_reg);
			LOG_MSG_DEBUG(I_LOG, "silead regulator_put success\n");
			fp_dev->fp_reg = NULL;
		}
	} else {
		if (gpio_is_valid(fp_dev->avdd_port)) {
...
int silfp_hw_poweron(struct silfp_data *fp_dev)
{
	int ret = 0;
	...
	if (fp_dev->power_type == 1) {
		...
		if (fp_dev->fp_reg != NULL) {
			...
			if (ret) {
			    ...
			    LOG_MSG_DEBUG(I_LOG, "----silead regulator_enable ok ----\n");
			}
		}
	} else {
		if (gpio_is_valid(fp_dev->avdd_port)) {
...
int silfp_hw_poweroff(struct silfp_data *fp_dev)
{
	int ret = 0;
	...
	if (fp_dev->power_type == 1) {
		...
		if (fp_dev->fp_reg != NULL){
			...
			if (ret) {
			    LOG_MSG_DEBUG(E_LOG, "%s regulator disable failed(%d)\n", __func__, ret);
			    goto err;
			} else {
			    LOG_MSG_DEBUG(I_LOG, "----silead regulator_disable ok ----\n");
			}
		}
	} else {
		if (gpio_is_valid(fp_dev->avdd_port)) {
