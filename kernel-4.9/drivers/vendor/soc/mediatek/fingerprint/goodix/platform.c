...
int gf_parse_dts(struct gf_dev *gf_dev)
{
	...
	gf_debug(INFO_LOG, "gf_dev->reset_gpio : %d\n", gf_dev->reset_gpio);
	...
	if (gpio_is_valid(gf_dev->reset_gpio)) {
		...
		if (rc) {
			...
			gf_debug(DEBUG_LOG, "success to request reset gpio!\n");
	...
	gf_debug(INFO_LOG, "gf_dev->irq_gpio : %d\n", gf_dev->irq_gpio);
	...
	if (gpio_is_valid(gf_dev->irq_gpio)) {
		...
		if (rc) {
			...
			gf_debug(DEBUG_LOG, "success to request irq gpio!\n");
	...
	if (gf_dev->power_type == 1) {
		...
		gf_debug(INFO_LOG, "gf_dev->pwr_gpio : %d\n", gf_dev->pwr_gpio);
		...
		if (gpio_is_valid(gf_dev->pwr_gpio)) {
			...
			if (rc) {
				...
				gf_debug(DEBUG_LOG, "success to request pwr gpio!\n");
...
int gf_cleanup(struct gf_dev *gf_dev)
{
	...
	if (gpio_is_valid(gf_dev->irq_gpio)) {
		gpio_free(gf_dev->irq_gpio);
		gf_debug(INFO_LOG, "gf remove irq_gpio success\n");
		gf_dev->irq_gpio = 0;
	}
	if (gpio_is_valid(gf_dev->reset_gpio)) {
		gpio_set_value(gf_dev->reset_gpio, 0);
		gpio_free(gf_dev->reset_gpio);
		gf_debug(INFO_LOG, "gf set reset low and remove reset_gpio success\n");
		gf_dev->reset_gpio = 0;
	}
	...
	if (gf_dev->power_type == 1) {
		...
		if (gpio_is_valid(gf_dev->pwr_gpio)) {
			...
			gf_debug(INFO_LOG, "gf set power low and remove pwr_gpio success\n");
			gf_dev->pwr_gpio = 0;
		}
	}
