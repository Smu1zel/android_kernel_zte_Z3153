...
static void hx852xG_chip_init(void)
{
	...
	*kp_CFG_VER_MAJ_FLASH_ADDR = 64384;  /*0xFCA0*/
...
{
	/*himax_parse_assign_cmd(hx852xg_data_df_rx,
	 *		on_pdriver_op->data_df_rx,
	 *		sizeof(on_pdriver_op->data_df_rx));
	 */
	/*himax_parse_assign_cmd(hx852xg_data_df_tx,
	 *		on_pdriver_op->data_df_tx,
	 *		sizeof(on_pdriver_op->data_df_tx));
	 */
	(*kp_private_ts)->ic_notouch_frame = hx852xg_notouch_frame;
	kp_himax_parse_assign_cmd(hx852xg_data_df_rx,
			(*kp_on_pdriver_op)->data_df_rx,
			sizeof((*kp_on_pdriver_op)->data_df_rx));
	kp_himax_parse_assign_cmd(hx852xg_data_df_tx,
			(*kp_on_pdriver_op)->data_df_tx,
			sizeof((*kp_on_pdriver_op)->data_df_tx));
	kp_himax_parse_assign_cmd(hx852xg_addr_fw_xy_rev_int_edge,
...
static bool hx852xG_chip_detect(void)
{
	...
	tmp_addr[0] = 0x5A;
	kp_himax_bus_write(0x80, tmp_addr, 1, HIMAX_I2C_RETRY_TIMES);
	I("MCU Sleep Out\n");

	kp_g_core_fp->fp_sense_off(true);

	kp_himax_bus_read(0x84, tmp_data, 4, HIMAX_I2C_RETRY_TIMES);
	I("R84H : [0]=0x%02X,[1]=0x%02X,[2]=0x%02X,[3]=0x%02X\n",
	tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);

	kp_himax_bus_read(0x96, tmp_data, 4, HIMAX_I2C_RETRY_TIMES);
	I("R96H : [0]=0x%02X,[1]=0x%02X,[2]=0x%02X,[3]=0x%02X\n",
	tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);
	...
	for (i = 0; i < 5; i++) {
		...
		if ((tmp_data[3] == 0x85) && (tmp_data[2] == 0x32)) {
			...
			strlcpy((*kp_private_ts)->chip_name,
				HX_85XX_G_SERIES_PWON, 30);
			(*kp_ic_data)->ic_adc_num = hx852xg_data_adc_num;
			break;
		}
	}
