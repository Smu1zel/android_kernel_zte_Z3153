...
static void himax_mcu_sense_on(uint8_t FlashMode)
{
	...
	tmp_data[0] = 0x0f;
	/*ROM ready*/
	ret = g_core_fp.fp_register_write(tmp_addr,
...
static void himax_mcu_resume_ic_action(void)
{
	...
	if (!(ts->open_one_key && ts->single_onekey_press)) {
	...
	int ret = -1;
		...
		if (suspended == 0) {
			if (state == 0x01) {
				...
				ret = g_core_fp.fp_register_write(on_pfw_op->addr_headline, DATA_LEN_4, tmp_data, 0);
				if (ret < 0) {
...
static bool himax_mrotation_set(uint8_t mrotation)
{
	...
	case mRotatin_0:
		himax_parse_assign_cmd(fw_data_safe_mode_release_pw_reset, tmp_data, 4);
		ret = g_core_fp.fp_register_write(on_pfw_op->addr_mrotation, DATA_LEN_4, tmp_data, 0);
		if (ret < 0) {
...
static void himax_mcu_pin_reset(void)
{
	...
	msleep(50);
	tmp_data[0] = 0x5A;
	himax_bus_write(0x80, tmp_data, 1, HIMAX_I2C_RETRY_TIMES);
	usleep_range(1000, 1100);
	tmp_data[0] = 0x00;
	himax_bus_write(0x80, tmp_data, 1, HIMAX_I2C_RETRY_TIMES);
	I("MCU Sleep Out\n");
