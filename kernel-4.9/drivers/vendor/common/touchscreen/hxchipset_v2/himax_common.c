...
uint8_t gest_event[GEST_SUP_NUM] = {
	0x80, 0x82, 0x83, 0x84, 0x85, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
...
uint16_t gest_key_def[GEST_SUP_NUM] = {
	KEY_GESTURE_DOUBLEC, HX_KEY_SINGLE_CLICK, HX_KEY_FP_DOWN,
	HX_KEY_FP_UP, HX_KEY_ONE_KEY, HX_KEY_UP, HX_KEY_DOWN, HX_KEY_LEFT,
...
static void himax_wake_event_report(void)
{
	...
	if (KEY_EVENT) {
		...
		if (KEY_EVENT == KEY_GESTURE_DOUBLEC) {
			UFP_INFO("Double Click Uevent\n");
			ufp_report_gesture_uevent(DOUBLE_TAP_GESTURE);
		} else if (KEY_EVENT == HX_KEY_SINGLE_CLICK) {
			UFP_INFO("Single Click Uevent\n");
			ufp_report_gesture_uevent(SINGLE_TAP_GESTURE);
		} else if (KEY_EVENT == HX_KEY_FP_DOWN) {
			UFP_INFO("Fingerprint Down Uevent\n");
			ts->single_onekey_press = true;
			report_ufp_uevent(UFP_FP_DOWN);
		} else if (KEY_EVENT == HX_KEY_FP_UP) {
			UFP_INFO("Fingerprint Up Uevent\n");
			ts->single_onekey_press = false;
			report_ufp_uevent(UFP_FP_UP);
		} else {
			UFP_INFO(" %s no support WAKEUP KEY event.\n", __func__);
		}
#else
		if (KEY_EVENT == KEY_GESTURE_DOUBLEC) {
...
static void himax_report_all_leave_event(struct himax_ts_data *ts)
{
	...
	for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
		...
		one_key_report(false, -1, -1, loop_i);
#endif
	}
...
static void himax_finger_report(struct himax_ts_data *ts)
{
	...
	for (i = 0; i < ts->nFinger_support; i++) {
		...
		if (valid) {
			...
			uf_touch_report(g_target_report_data->x[i],
					 g_target_report_data->y[i],
					i);
#endif
#if defined(CONFIG_TPD_UFP_MAC) && defined(ZTE_ONE_KEY)
...
static void himax_report_points(struct himax_ts_data *ts)
{
	if (g_ts_dbg != 0)
		I("%s: start!\n", __func__);
#ifdef CONFIG_TPD_UFP_MAC
	if (ts->gesture_cust_en[1]) {
		if ((hx_touch_data->hx_state_info[1] >> 6) & 0x01) {
			report_ufp_uevent(UFP_FP_DOWN);
		}

		if ((hx_touch_data->hx_state_info[1] >> 7) & 0x01) {
			report_ufp_uevent(UFP_FP_UP);
		}
	}
#endif
...
static void himax_boot_upgrade(struct work_struct *work)
{
	...
	if (g_core_fp.fp_bin_desc_get != NULL)
