...
extern int gtp_get_vendor_and_firmware(struct goodix_ts_core *core_data);
...
static int goodix_ts_power_init(struct goodix_ts_core *core_data)
{
	...
	regulator_set_voltage(core_data->avdd, 3000000, 3100000);
	return r;
}

static void gooix_regulator_deinit(struct goodix_ts_core *core_data)
{

	ts_info("%s: entered.\n", __func__);

	if (!IS_ERR(core_data->iovdd)) {
		regulator_disable(core_data->iovdd);
		regulator_put(core_data->iovdd);
	}
	if (!IS_ERR(core_data->avdd))
		regulator_disable(core_data->avdd);
		regulator_put(core_data->avdd);

	ts_info("%s: regulator deinit, completed.\n", __func__);
...
int goodix_ts_power_on(struct goodix_ts_core *core_data)
{
	...
	if (core_data->iovdd) {
		...
		if (r)
			ts_err("Failed to enable iovdd power:%d", r);
			return r;
	}
...
int goodix_ts_power_off(struct goodix_ts_core *core_data)
{
	...
	if (core_data->iovdd) {
		...
		if (r) {
			ts_err("Failed to disable iovdd power:%d", r);
			return r;
		}
	}
...
static void goodix_ts_gpio_free(struct goodix_ts_core *core_data)
{
	struct goodix_ts_board_data *ts_bdata = board_data(core_data);

	if (gpio_is_valid(ts_bdata->reset_gpio))
		devm_gpio_free(&core_data->pdev->dev, ts_bdata->reset_gpio);
	if (gpio_is_valid(ts_bdata->irq_gpio))
		devm_gpio_free(&core_data->pdev->dev, ts_bdata->irq_gpio);
}
...
static int goodix_ts_resume(void *_core_data)
{
	...
	goodix_set_edge_suppress(core_data->zc.edge_limit_level, 0, core_data);
...
static void goodix_ts_probe(struct work_struct *work)
{
	...
	r = gtp_get_vendor_and_firmware(core_data);
	if (r) {
		ts_err("get cfg firmware error ,use default cfg");
	}
	...
	return;
free_gpio:
	goodix_ts_gpio_free(core_data);
out:
	gooix_regulator_deinit(core_data);
	core_data->initialized = 0;
	ts_info("goodix_ts_probe ERROR OUT, r:%d", r);
}
