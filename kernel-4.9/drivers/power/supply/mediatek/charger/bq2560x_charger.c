...
static int bq2560x_set_watch_dog_timer(struct charger_device *chg_dev, u32 timer)
{
	u8 reg_value = 0;

	pr_info("bq2560x: [REG] set watchdog timer %ds\n", timer);

	reg_value = (timer - BQ2560X_REG05_WDT_BASE) / BQ2560X_REG05_WDT_LSB;
	bq2560x_set_value(BQ2560X_REG_05, BQ2560X_REG05_WDT_MASK,
			  BQ2560X_REG05_WDT_SHIFT, reg_value);
...
{
	bq2560x_set_watch_dog_timer(chg_dev, 40);
...
{
	bq2560x_set_watch_dog_timer(chg_dev, 0);
...
static unsigned int bq2560x_hw_init(struct charger_device *chg_dev)
{
	...
	bq2560x_set_cv_voltage(chg_dev, 4400000);	/* VREG 4.40V */
