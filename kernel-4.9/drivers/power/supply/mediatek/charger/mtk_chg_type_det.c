	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		val->intval = zte_charger_notify_usb_current_max();
		pr_info("mtk usb max current is %d\n", val->intval);
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		val->intval = zte_charger_notify_usb_voltage_max();
		pr_info("mtk usb max voltage is %d\n", val->intval);
		break;
	default:
		return -EINVAL;
	}
