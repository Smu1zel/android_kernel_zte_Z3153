...
extern u32 g_input_current; /* uA */
extern u32 g_topoff_current; /* uA */
extern u32 g_recharger_voltage; /* uV */
#endif
...
static bool charging_full_check(struct charger_manager *info)
{
	...
	pr_info("chg_current = %d, g_topoff_current=%d\n", chg_current, g_topoff_current);
	if (chg_current > g_topoff_current)
		full_check_count = 0;
#else
...
static int mtk_linear_chr_full(struct charger_manager *info)
{
	...
	if (info->enable_sw_jeita && info->sw_jeita.cv != 0) {
		if (vbat < (info->sw_jeita.cv - g_recharger_voltage))
			is_recharging = true;
	} else {
		if (vbat < (info->data.battery_cv - g_recharger_voltage))
			is_recharging = true;
	}
#else
	if (info->enable_sw_jeita && info->sw_jeita.cv != 0) {
