...
int VBAT_to_OCV_c(int _vbat)
{
	struct FUELGAUGE_PROFILE_STRUCT *profile_p;
	unsigned short zte_voltage[100];
	int ret_vol = 0;
	int i = 0, size, high;
	int ptim_i = get_ptim_i();
	...
	for (i = 0; i < size; i++) {
		zte_voltage[i] = profile_p[i].voltage + (ptim_i * profile_p[i].resistance) / 10000;
	}

	for (i = 0; i < size; i++) {
		bm_err(
		"(%2d,%5d,%5d,%5d,%3d)\n",
		i, profile_p[i].mah, zte_voltage[i],
		profile_p[i].resistance, profile_p[i].percentage);
	}

	for (i = 0; i < size; i++) {
		if (zte_voltage[i] <= _vbat)
			break;
	}
	...
	bm_err("[FGADC] VBAT_to_OCV: vbat:%d ocv:%d highidx:%d\n",
		_vbat, ret_vol, high);
...
{
	int init_swvbat = get_ptim_vbat();
	int init_swocv = init_swvbat;
	int con0_soc = get_con0_soc();
	int con0_uisoc = get_rtc_ui_soc();

	init_swocv = VBAT_to_OCV_c(init_swvbat);
