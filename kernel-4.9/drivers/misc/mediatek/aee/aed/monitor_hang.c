...
extern int kill_something_info(int sig, struct siginfo *info, pid_t pid);
...
int zte_hangflag = 0;
module_param_named(hangflag, zte_hangflag, uint, 0644);
...
static struct task_struct* FindTaskStructByName(char *name) {
	struct task_struct *task;
...
static int hang_detect_thread(void *arg)
{
	...
	struct task_struct *server_thread = NULL;
	...
	while (1) {
        ...
        if( (zte_hangflag == 1) && (hd_detect_enabled == 1) && (hd_timeout == 10 ) && (hang_detect_counter == 5)) {
			server_thread = FindTaskStructByName("system_server");
			if (server_thread != NULL && system_server_pid != -1) {
				struct siginfo info;
				send_sig( 18 ,server_thread ,0 );
				msleep(5 * 1000);
				info.si_signo = 3;
				info.si_errno = 0;
				...
				kill_something_info(3, &info, system_server_pid);
			}
        }
		if (reboot_flag || ((hd_detect_enabled == 1) &&
			(system_server_pid != -1)))
