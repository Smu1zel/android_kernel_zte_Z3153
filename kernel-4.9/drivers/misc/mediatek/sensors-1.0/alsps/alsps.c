...
int pshub_data_report_t(int *value, int status, int64_t time_stamp)
{
	int err = 0;
	struct sensor_event event;
	...
	pr_err("[ALS/PS]%s! %d, %d, %d, %d\n", __func__, value[0], value[1], value[2], status);
	event.flush_action = DATA_ACTION;
	event.time_stamp = time_stamp;
	event.word[0] = value[0] + 1;
	event.word[1] = (value[1] & 0xFFFF0000) >> 16;
	event.word[2] = (value[1] & 0xFFFF);
	event.word[3] = (value[2] & 0xFFFF0000) >> 16;
	event.word[4] = (value[2] & 0xFFFF);
	event.status = status;
