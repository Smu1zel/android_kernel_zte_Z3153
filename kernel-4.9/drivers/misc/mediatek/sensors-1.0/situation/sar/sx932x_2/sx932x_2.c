...
static int sx932x_Hardware_Check(psx93XX_t this)
{
	...
	dev_info(this->pdev, "sx932x_2 status = 0x%x\n", this->failStatusCode);
...
{
	situation_flush_report(ID_SAR_2);
	return 0;
}
...
static int sx932x_i2c_probe(struct i2c_client *client,  const struct i2c_device_id *id)
{
	...
	extern int sensorlist_register_deviceinfo(int sensor, char *devName);
	...
	err = situation_register_control_path(&ctl, ID_SAR_2);
	if (err) {
	...
	err = situation_register_data_path(&data, ID_SAR_2);
	if (err) {
		pr_err("register sar data path err\n");
		return err;
	}

	sensorlist_register_deviceinfo(ID_SAR_2, "sx9325_2");
	pthis = this;
...
static int __init sar_sx932x_2_init(void)
{
	situation_driver_add(&sar_sx932x_init_info, ID_SAR_2);
	return 0;
}

static void __exit sar_sx932x_2_exit(void)
{
	pr_err("%s\n", __func__);
}

module_init(sar_sx932x_2_init);
module_exit(sar_sx932x_2_exit);

MODULE_DESCRIPTION("SX932x_2 Capacitive Touch Controller Driver");
MODULE_LICENSE("GPL");
MODULE_VERSION("0.1");
