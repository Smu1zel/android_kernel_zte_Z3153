	...
	bool is_increase)
{
	...
	/* MTK PE+ increasing curve, contain four parts*/
	/* section 1 */
	for (i = 0; i < 1; i++) {
		msleep(100);
...
static int mt6370_send_pep20_current_logic1_zte(struct mt6370_pmu_charger_data *chg_data)
{
	int ret = 0;
...
static int mt6370_send_pep20_current_logic0_zte(struct mt6370_pmu_charger_data *chg_data)
{
	int ret = 0;
...
static int mt6370_send_pep20_current_start_zte(struct mt6370_pmu_charger_data *chg_data)
{
	int ret = 0;
...
static int mt6370_send_pep20_current_end_zte(struct mt6370_pmu_charger_data *chg_data)
{
	int ret = 0;
...
static int mt6370_set_pep20_current_pattern_zte(struct charger_device *chg_dev,
	u32 uV)
{
	int ret = 0, i = 0;
	bool bit[5] = {0};
	...
	ret = mt6370_send_pep20_current_start_zte(chg_data);
	if (ret < 0)
		goto out;

	for (i = 4; i >= 0; i--) {
		bit[i] = (reg_volt >> i) & 0x01;
		if (bit[i] == true) {
			ret = mt6370_send_pep20_current_logic1_zte(chg_data);
			if (ret < 0)
				goto out;
		} else {
			ret = mt6370_send_pep20_current_logic0_zte(chg_data);
			if (ret < 0)
				goto out;
		}
	}

	ret = mt6370_send_pep20_current_end_zte(chg_data);
	ret = (ret >= 0) ? 0 : ret;
out:
	mutex_unlock(&chg_data->pe_access_lock);
	return ret;
...
static int mt6370_do_event(struct charger_device *chg_dev, u32 event, u32 args)
{
	...
	if (chg_data->chg_type == CHARGER_UNKNOWN) {
		dev_info(chg_data->dev, "%s: ignore charger_dev_notify, when charger plug out\n", __func__);
		return 0;
	}
#endif
...
static struct charger_ops mt6370_chg_ops = {
	...
	.send_ta20_current_pattern_zte = mt6370_set_pep20_current_pattern_zte,
