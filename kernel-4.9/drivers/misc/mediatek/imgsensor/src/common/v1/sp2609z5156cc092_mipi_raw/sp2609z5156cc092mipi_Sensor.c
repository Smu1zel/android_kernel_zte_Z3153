...
#include "sp2609z5156cc092mipi_Sensor.h"

#define PFX "SP2609Z5156CC092_camera_sensor"
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	if (gain >= BASEGAIN && gain <= 16*BASEGAIN) {
		...
		if (iReg <= 0x10) {
			...
			write_cmos_sensor(0x01, 0x01);
			LOG_INF("SP2609Z5156CC092MIPI_SetGain = 16");
		} else if (iReg >= 0xf8)/*huazai*/ {
			write_cmos_sensor(0xfd, 0x01);
			write_cmos_sensor(0x24, 0xf8);
			write_cmos_sensor(0x01, 0x01);
			LOG_INF("SP2609Z5156CC092MIPI_SetGain = 160");
		} else {
			write_cmos_sensor(0xfd, 0x01);
			write_cmos_sensor(0x24, (kal_uint8)iReg);
			write_cmos_sensor(0x01, 0x01);
			LOG_INF("sp2609z5156cc09211111-------SP2609Z5156CC092MIPI_SetGain ireg = 0x%x,gain = 0x%x\n",
				iReg, gain);
		}
	} else {
...
UINT32 SP2609Z5156CC092_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc != NULL)
		*pfFunc = &sensor_func;
	return ERROR_NONE;
}    /*    SP2609Z5156CC092_MIPI_SensorInit    */
