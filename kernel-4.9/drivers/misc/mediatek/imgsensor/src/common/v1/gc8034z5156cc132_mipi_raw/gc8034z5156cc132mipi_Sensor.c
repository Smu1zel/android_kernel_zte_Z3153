...
#include "gc8034z5156cc132mipi_Sensor.h"

/************************** Modify Following Strings for Debug **************************/
#define PFX "gc8034z5156cc132_camera_sensor"
#define LOG_1 LOG_INF("GC8034Z5156CC132, MIPI 4LANE\n")
/****************************   Modify end    *******************************************/
#define GC8034Z5156CC132_DEBUG    1
#if GC8034Z5156CC132_DEBUG
...
#if GC8034Z5156CC132_MIRROR_FLIP_ENABLE
...
struct gc8034z5156cc132_otp_t gc8034z5156cc132_otp_info;

static kal_uint8 gc8034z5156cc132_read_otp(kal_uint8 page, kal_uint8 addr)
{
	LOG_INF("E!\n");
...
static void gc8034z5156cc132_read_otp_kgroup(kal_uint8 page, kal_uint8 addr, kal_uint8 *buff, int size)
{
	kal_uint16 i;
...
static void gc8034z5156cc132_read_otp_group(kal_uint8 page, kal_uint8 addr, kal_uint8 *buff, int size)
{
	kal_uint8 i;
	kal_uint8 regf4 = read_cmos_sensor(0xf4);
...
void Gc8034_Send_AF_OTP(u8 *data, u32 size)
{
	...
	LOG_INF("GC8034Z5156CC132_OTP_AF:infitiy=0x%x\n", gc8034z5156cc132_otp_info.af_infinity);
	LOG_INF("GC8034Z5156CC132_OTP_AF:macro=0x%x\n", gc8034z5156cc132_otp_info.af_macro);
	af_otp_info |= gc8034z5156cc132_otp_info.af_infinity << 16;
	af_otp_info |= gc8034z5156cc132_otp_info.af_macro;
	LOG_INF("GC8034Z5156CC132_OTP_AF:af_otp_info=0x%x\n", af_otp_info);
	for (i = 0; i < size; i++) {
		*(data+i) = (af_otp_info >> (8 * i)) & 0xFF;
	}
}

static void gc8034z5156cc132_gcore_read_otp_info(void)
{
	kal_uint8  flagdd = 0;
	kal_uint16 i = 0, j = 0;
	kal_uint8  temp = 0;
...
#if GC8034Z5156CC132OTP_FOR_CUSTOMER
	kal_uint32 check = 0;
...
#endif
#if GC8034Z5156CC132OTP_DEBUG
	kal_uint8  debug[128] = { 0 };
#endif
	memset(&gc8034z5156cc132_otp_info, 0, sizeof(struct gc8034z5156cc132_otp_t));

	/* Static Defective Pixel */
	flagdd = gc8034z5156cc132_read_otp(0, 0x0b);
	LOG_INF("flag_dd = 0x%x\n", flagdd);

	switch (flagdd & 0x03) {
	case 0x00:
		LOG_INF("DD is empty!\n");
		gc8034z5156cc132_otp_info.dd_flag = 0x00;
		break;
	case 0x01:
		LOG_INF("DD is valid!\n");
		total_number = gc8034z5156cc132_read_otp(0, 0x0c) + gc8034z5156cc132_read_otp(0, 0x0d);
		gc8034z5156cc132_otp_info.dd_flag = 0x01;
		gc8034z5156cc132_read_otp_kgroup(0, 0x0e, &ddtempbuff[0], 4 * total_number);
		for (i = 0; i < total_number; i++) {
			if ((ddtempbuff[4 * i + 3] & 0x80) == 0x80) {
				if ((ddtempbuff[4 * i + 3] & 0x03) == 0x03) {
					gc8034z5156cc132_otp_info.dd_param[cnt].x =
						(((kal_uint16)ddtempbuff[4 * i + 1] & 0x0f) << 8) +
						ddtempbuff[4 * i];
					gc8034z5156cc132_otp_info.dd_param[cnt].y =
						((kal_uint16)ddtempbuff[4 * i + 2] << 4) +
						((ddtempbuff[4 * i + 1] & 0xf0) >> 4);
					gc8034z5156cc132_otp_info.dd_param[cnt++].t = 2;
					gc8034z5156cc132_otp_info.dd_param[cnt].x =
						(((kal_uint16)ddtempbuff[4 * i + 1] & 0x0f) << 8) +
						ddtempbuff[4 * i];
					gc8034z5156cc132_otp_info.dd_param[cnt].y =
					...
					gc8034z5156cc132_otp_info.dd_param[cnt].y =
						((kal_uint16)ddtempbuff[4 * i + 2] << 4) +
						((ddtempbuff[4 * i + 1] & 0xf0) >> 4);
					gc8034z5156cc132_otp_info.dd_param[cnt++].t =
						ddtempbuff[4 * i + 3] & 0x03;
				}
			}
		}
		gc8034z5156cc132_otp_info.dd_cnt = cnt;
		LOG_INF("DD total_number = %d\n", gc8034z5156cc132_otp_info.dd_cnt);
		break;
	case 0x02:
	case 0x03:
		LOG_INF("DD is invalid!\n");
		gc8034z5156cc132_otp_info.dd_flag = 0x02;
		break;
	default:
		break;
	}
...
#if GC8034Z5156CC132OTP_FOR_CUSTOMER
	flag_module = gc8034z5156cc132_read_otp(9, 0x6f);
	flag_wb = gc8034z5156cc132_read_otp(9, 0x5e);
	flag_af = gc8034z5156cc132_read_otp(3, 0x3a);
	...
	for (index = 0; index < 2; index++) {
		...
		case 0x04:
			...
			check = 0;
			gc8034z5156cc132_read_otp_group(9, INFO_ROM_START + index * INFO_WIDTH,
				&info[0], INFO_WIDTH);
			for (i = 0; i < INFO_WIDTH - 1; i++)
				check += info[i];

			if ((check % 255 + 1) == info[INFO_WIDTH - 1]) {
				gc8034z5156cc132_otp_info.module_id = info[0];
				gc8034z5156cc132_otp_info.lens_id = info[1];
				gc8034z5156cc132_otp_info.vcm_driver_id = info[2];
				gc8034z5156cc132_otp_info.vcm_id = info[3];
				gc8034z5156cc132_otp_info.year = info[4];
				gc8034z5156cc132_otp_info.month = info[5];
				gc8034z5156cc132_otp_info.day = info[6];
			} else
		...
		case 0x00:
			...
			gc8034z5156cc132_otp_info.wb_flag = gc8034z5156cc132_otp_info.wb_flag | 0x00;
			break;
		case 0x04:
			LOG_INF("wb current group %d is valid!\n", index + 1);
			check = 0;
			gc8034z5156cc132_read_otp_group(9, WB_ROM_START + index * WB_WIDTH, &wb[0], WB_WIDTH);
			for (i = 0; i < WB_WIDTH - 1; i++)
				check += wb[i];

			if ((check % 255 + 1) == wb[WB_WIDTH - 1]) {
				gc8034z5156cc132_otp_info.rg_gain = (wb[0] | ((wb[1] & 0xf0) << 4)) > 0 ?
					(wb[0] | ((wb[1] & 0xf0) << 4)) : 0x400;
				gc8034z5156cc132_otp_info.bg_gain = (((wb[1] & 0x0f) << 8) | wb[2]) > 0 ?
					(((wb[1] & 0x0f) << 8) | wb[2]) : 0x400;
				gc8034z5156cc132_otp_info.wb_flag = gc8034z5156cc132_otp_info.wb_flag | 0x01;
			} else
		...
		case 0x0c:
			...
			gc8034z5156cc132_otp_info.wb_flag = gc8034z5156cc132_otp_info.wb_flag | 0x02;
			break;
		default:
			break;
		}
		...
		case 0x00:
			...
			gc8034z5156cc132_otp_info.golden_flag = gc8034z5156cc132_otp_info.golden_flag | 0x00;
			break;
		case 0x40:
			LOG_INF("wb golden group %d is valid!\n", index + 1);
			check = 0;
			gc8034z5156cc132_read_otp_group(9, GOLDEN_ROM_START + index * GOLDEN_WIDTH,
				&golden[0], GOLDEN_WIDTH);
			for (i = 0; i < GOLDEN_WIDTH - 1; i++)
				check += golden[i];

			if ((check % 255 + 1) == golden[GOLDEN_WIDTH - 1]) {
				gc8034z5156cc132_otp_info.golden_rg = (golden[0] | ((golden[1] & 0xf0) << 4)) > 0 ?
					(golden[0] | ((golden[1] & 0xf0) << 4)) : RG_TYPICAL;
				gc8034z5156cc132_otp_info.golden_bg = (((golden[1] & 0x0f) << 8) | golden[2]) > 0 ?
					(((golden[1] & 0x0f) << 8) | golden[2]) : BG_TYPICAL;
				gc8034z5156cc132_otp_info.golden_flag = gc8034z5156cc132_otp_info.golden_flag | 0x01;
			} else
		...
		case 0xc0:
			...
			gc8034z5156cc132_otp_info.golden_flag = gc8034z5156cc132_otp_info.golden_flag | 0x02;
			break;
		default:
			break;
		}
		...
		case 0x00:
			...
			gc8034z5156cc132_otp_info.af_flag = gc8034z5156cc132_otp_info.af_flag | 0x00;
			break;
		case 0x04:
			LOG_INF("af group %d is valid!\n", index + 1);
			check = 0;
			gc8034z5156cc132_read_otp_group(3, AF_ROM_START + index * AF_WIDTH, &af[0], AF_WIDTH);
			for (i = 0; i < AF_WIDTH - 1; i++)
				check += af[i];

			if ((check % 255 + 1) == af[AF_WIDTH - 1]) {
				gc8034z5156cc132_otp_info.af_infinity = ((af[0] << 4) & 0x0f00) + af[1];
				gc8034z5156cc132_otp_info.af_macro = ((af[0] << 8) & 0x0f00) + af[2];
				gc8034z5156cc132_otp_info.af_flag = gc8034z5156cc132_otp_info.af_flag | 0x01;
			} else
		...
		case 0x0c:
			...
			gc8034z5156cc132_otp_info.af_flag = gc8034z5156cc132_otp_info.af_flag | 0x02;
			break;
		default:
			break;
		}
	...
	/* LSC */
	flag_lsc = gc8034z5156cc132_read_otp(3, 0x43);
	LOG_INF("flag_lsc = 0x%x\n", flag_lsc);
	gc8034z5156cc132_otp_info.lsc_flag = 0;
	for (index = 0; index < 2; index++) {
		switch ((flag_lsc << (2 * index)) & 0x0c) {
		case 0x00:
			LOG_INF("lsc group %d is empty!\n", index + 1);
			gc8034z5156cc132_otp_info.lsc_flag = gc8034z5156cc132_otp_info.lsc_flag | 0x00;
			break;
		case 0x04:
			LOG_INF("lsc group %d is valid!\n", index + 1);
			check = 0;
			if (index == 0) {
				gc8034z5156cc132_read_otp_kgroup(3, 0x44, &gc8034z5156cc132_otp_info.lsc_param[0], 396);
				lsc_checksum = gc8034z5156cc132_read_otp(6, 0x50);
			} else {
				gc8034z5156cc132_read_otp_kgroup(6, 0x51, &gc8034z5156cc132_otp_info.lsc_param[0], 396);
				lsc_checksum = gc8034z5156cc132_read_otp(9, 0x5d);
			}
			for (i = 0; i < 396; i++)
				check += gc8034z5156cc132_otp_info.lsc_param[i];
			if ((check % 255 + 1) == lsc_checksum)
				gc8034z5156cc132_otp_info.lsc_flag = gc8034z5156cc132_otp_info.lsc_flag | 0x01;
			else {
				gc8034z5156cc132_otp_info.lsc_flag = gc8034z5156cc132_otp_info.lsc_flag | 0x02;
	...
	LOG_INF("module_id = 0x%x\n", gc8034z5156cc132_otp_info.module_id);
	LOG_INF("lens_id = 0x%x\n", gc8034z5156cc132_otp_info.lens_id);
	LOG_INF("vcm_id = 0x%x\n", gc8034z5156cc132_otp_info.vcm_id);
	LOG_INF("vcm_driver_id = 0x%x\n", gc8034z5156cc132_otp_info.vcm_driver_id);
	LOG_INF("data = %d-%d-%d\n", gc8034z5156cc132_otp_info.year,
		gc8034z5156cc132_otp_info.month, gc8034z5156cc132_otp_info.day);
	LOG_INF("r/g = 0x%x\n", gc8034z5156cc132_otp_info.rg_gain);
	LOG_INF("b/g = 0x%x\n", gc8034z5156cc132_otp_info.bg_gain);
	LOG_INF("golden_rg = 0x%x\n", gc8034z5156cc132_otp_info.golden_rg);
	LOG_INF("golden_bg = 0x%x\n", gc8034z5156cc132_otp_info.golden_bg);
	LOG_INF("af_infitiy = 0x%x\n", gc8034z5156cc132_otp_info.af_infinity);
	LOG_INF("af_macro = 0x%x\n", gc8034z5156cc132_otp_info.af_macro);
#endif

	/* chip regs */
	gc8034z5156cc132_otp_info.reg_flag = gc8034z5156cc132_read_otp(2, 0x4e);

	if (gc8034z5156cc132_otp_info.reg_flag == 1)
		for (i = 0; i < 5; i++) {
			temp = gc8034z5156cc132_read_otp(2, 0x4f + 5 * i);
			for (j = 0; j < 2; j++)
				if (((temp >> (4 * j + 3)) & 0x01) == 0x01) {
					gc8034z5156cc132_otp_info.reg_page[gc8034z5156cc132_otp_info.reg_num] =
						(temp >> (4 * j)) & 0x03;
					gc8034z5156cc132_otp_info.reg_addr[gc8034z5156cc132_otp_info.reg_num] =
						gc8034z5156cc132_read_otp(2, 0x50 + 5 * i + 2 * j);
					gc8034z5156cc132_otp_info.reg_value[gc8034z5156cc132_otp_info.reg_num] =
						gc8034z5156cc132_read_otp(2, 0x50 + 5 * i + 2 * j + 1);
					gc8034z5156cc132_otp_info.reg_num++;
				}
		}

#if GC8034Z5156CC132OTP_DEBUG
	for (i = 0; i < 10; i++) {
		gc8034z5156cc132_read_otp_group(i, 0, &debug[0], 128);
		for (j = 0; j < 128; j++)
...
static void gc8034z5156cc132_gcore_check_prsel(void)
{
	kal_uint8 product_level = 0;

	product_level = gc8034z5156cc132_read_otp(2, 0x68) & 0x07;
...
static void gc8034z5156cc132_gcore_update_dd(void)
{
	kal_uint8 i = 0, j = 0;
	kal_uint8 temp_val0 = 0, temp_val1 = 0, temp_val2 = 0;
	struct gc8034z5156cc132_dd_t dd_temp = {0, 0, 0};

	if (gc8034z5156cc132_otp_info.dd_flag == 0x01) {
		LOG_INF("auto load start!\n");
		for (i = 0; i < gc8034z5156cc132_otp_info.dd_cnt; i++) {
#if GC8034Z5156CC132_MIRROR_FLIP_ENABLE
			switch (gc8034z5156cc132_otp_info.dd_param[i].t) {
			case 0:
				gc8034z5156cc132_otp_info.dd_param[i].x =
					DD_WIDTH - gc8034z5156cc132_otp_info.dd_param[i].x + 1;
				break;
			case 1:
				gc8034z5156cc132_otp_info.dd_param[i].x =
					DD_WIDTH - gc8034z5156cc132_otp_info.dd_param[i].x - 1;
			...
			default:
				gc8034z5156cc132_otp_info.dd_param[i].x =
					DD_WIDTH - gc8034z5156cc132_otp_info.dd_param[i].x;
				break;
			}
#endif
#if GC8034Z5156CC132_MIRROR_FLIP_ENABLE
			gc8034z5156cc132_otp_info.dd_param[i].y =
				DD_HEIGHT - gc8034z5156cc132_otp_info. dd_param[i].y + 1;
#endif
		}
		for (i = 0; i < gc8034z5156cc132_otp_info.dd_cnt - 1; i++) {
			for (j = i + 1; j < gc8034z5156cc132_otp_info.dd_cnt; j++) {
				if (gc8034z5156cc132_otp_info.dd_param[i].y * DD_WIDTH
					+ gc8034z5156cc132_otp_info.dd_param[i].x
					> gc8034z5156cc132_otp_info.dd_param[j].y * DD_WIDTH
					+ gc8034z5156cc132_otp_info.dd_param[j].x) {
					dd_temp.x = gc8034z5156cc132_otp_info.dd_param[i].x;
					dd_temp.y = gc8034z5156cc132_otp_info.dd_param[i].y;
					dd_temp.t = gc8034z5156cc132_otp_info.dd_param[i].t;
					gc8034z5156cc132_otp_info.dd_param[i].x =
						gc8034z5156cc132_otp_info.dd_param[j].x;
					gc8034z5156cc132_otp_info.dd_param[i].y =
						gc8034z5156cc132_otp_info.dd_param[j].y;
					gc8034z5156cc132_otp_info.dd_param[i].t =
						gc8034z5156cc132_otp_info.dd_param[j].t;
					gc8034z5156cc132_otp_info.dd_param[j].x = dd_temp.x;
					gc8034z5156cc132_otp_info.dd_param[j].y = dd_temp.y;
					gc8034z5156cc132_otp_info.dd_param[j].t = dd_temp.t;
				}
			}
		}
		...
		for (i = 0; i < gc8034z5156cc132_otp_info.dd_cnt; i++) {
			temp_val0 = gc8034z5156cc132_otp_info.dd_param[i].x & 0x00ff;
			temp_val1 = ((gc8034z5156cc132_otp_info.dd_param[i].y & 0x000f) << 4)
				+((gc8034z5156cc132_otp_info.dd_param[i].x & 0x0f00)>>8);
			temp_val2 = (gc8034z5156cc132_otp_info.dd_param[i].y & 0x0ff0) >> 4;
			write_cmos_sensor(0xaa, i);
			write_cmos_sensor(0xac, temp_val0);
			write_cmos_sensor(0xac, temp_val1);
			write_cmos_sensor(0xac, temp_val2);
			write_cmos_sensor(0xac, gc8034z5156cc132_otp_info.dd_param[i].t);

			LOG_INF("GC8034Z5156CC132_OTP_GC val0 = 0x%x, val1 = 0x%x, val2 = 0x%x\n",
				temp_val0, temp_val1, temp_val2);
			LOG_INF("GC8034Z5156CC132_OTP_GC x = %d , y = %d\n", ((temp_val1&0x0f)<<8) +
...
#if GC8034Z5156CC132OTP_FOR_CUSTOMER
static void gc8034z5156cc132_gcore_update_wb(void)
{
	...
	if (0x01 == (gc8034z5156cc132_otp_info.golden_flag & 0x01)) {
		rg_typical = gc8034z5156cc132_otp_info.golden_rg;
		bg_typical = gc8034z5156cc132_otp_info.golden_bg;
	...
	if (0x01 == (gc8034z5156cc132_otp_info.wb_flag & 0x01)) {
		r_gain_current = 2048 * rg_typical / gc8034z5156cc132_otp_info.rg_gain;
		b_gain_current = 2048 * bg_typical / gc8034z5156cc132_otp_info.bg_gain;
		g_gain_current = 2048;
...
static void gc8034z5156cc132_gcore_update_lsc(void)
{
	kal_uint8 i = 0, j = 0;
	kal_uint16 base = 0;

	if ((gc8034z5156cc132_otp_info.lsc_flag & 0x01) == 0x01) {
		write_cmos_sensor(0xfe, 0x01);
		...
		for (i = 0; i < 9; i++) {
			...
			for (j = 0; j < 11; j++) {
				...
				base = 4 * (11 * i + j);
#endif
				write_cmos_sensor(0xcc, gc8034z5156cc132_otp_info.lsc_param[base + 0]);
				write_cmos_sensor(0xcc, gc8034z5156cc132_otp_info.lsc_param[base + 1]);
				write_cmos_sensor(0xcc, gc8034z5156cc132_otp_info.lsc_param[base + 2]);
				write_cmos_sensor(0xcc, gc8034z5156cc132_otp_info.lsc_param[base + 3]);
		}
		}
...
static void gc8034z5156cc132_gcore_update_chipversion(void)
{
	kal_uint8 i = 0;

	LOG_INF("reg_num = %d\n", gc8034z5156cc132_otp_info.reg_num);

	if (gc8034z5156cc132_otp_info.reg_flag)
		for (i = 0; i < gc8034z5156cc132_otp_info.reg_num; i++) {
			write_cmos_sensor(0xfe, gc8034z5156cc132_otp_info.reg_page[i]);
			write_cmos_sensor(gc8034z5156cc132_otp_info.reg_addr[i],
				gc8034z5156cc132_otp_info.reg_value[i]);
			LOG_INF("P%d:0x%x -> 0x%x\n",
			gc8034z5156cc132_otp_info.reg_page[i], gc8034z5156cc132_otp_info.reg_addr[i],
				gc8034z5156cc132_otp_info.reg_value[i]);
		}
}

static void gc8034z5156cc132_gcore_update_otp(void)
{
	gc8034z5156cc132_gcore_update_dd();
	gc8034z5156cc132_gcore_check_prsel();
#if GC8034Z5156CC132OTP_FOR_CUSTOMER
	gc8034z5156cc132_gcore_update_wb();
	gc8034z5156cc132_gcore_update_lsc();
#endif
	gc8034z5156cc132_gcore_update_chipversion();
}

static void gc8034z5156cc132_otp_identify(void)
{
	memset(&gc8034z5156cc132_otp_info, 0, sizeof(gc8034z5156cc132_otp_info));
	...
	gc8034z5156cc132_read_otp_group(0, GC8034Z5156CC132_OTP_ID_DATA_OFFSET,
		&gc8034z5156cc132_otp_info.otp_id[0], GC8034Z5156CC132_OTP_ID_SIZE);
	gc8034z5156cc132_gcore_read_otp_info();
...
static void gc8034z5156cc132_otp_function(void)
{
	kal_uint8 i = 0, flag = 0;
	kal_uint8 otp_id[GC8034Z5156CC132_OTP_ID_SIZE];

	memset(&otp_id, 0, GC8034Z5156CC132_OTP_ID_SIZE);
	write_cmos_sensor(0xfc, 0x00);
	write_cmos_sensor(0xf7, 0x97);
	write_cmos_sensor(0xfc, 0x00);
	write_cmos_sensor(0xfc, 0x00);
	...
	write_cmos_sensor(0xf4, 0x88);
	gc8034z5156cc132_read_otp_group(0, GC8034Z5156CC132_OTP_ID_DATA_OFFSET,
		&otp_id[0], GC8034Z5156CC132_OTP_ID_SIZE);
	for (i = 0; i < GC8034Z5156CC132_OTP_ID_SIZE; i++)
		if (otp_id[i] != gc8034z5156cc132_otp_info.otp_id[i]) {
			flag = 1;
			break;
		}
	...
	if (flag == 1) {
		...
		memset(&gc8034z5156cc132_otp_info, 0, sizeof(gc8034z5156cc132_otp_info));
		for (i = 0; i < GC8034Z5156CC132_OTP_ID_SIZE; i++)
			gc8034z5156cc132_otp_info.otp_id[i] = otp_id[i];
		gc8034z5156cc132_gcore_read_otp_info();
	}

	gc8034z5156cc132_gcore_update_otp();
	write_cmos_sensor(0xf2, 0x00);
	write_cmos_sensor(0xf4, 0x80);
	write_cmos_sensor(0xfc, 0x00);
	write_cmos_sensor(0xf7, 0x95);
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x17, GC8034Z5156CC132_MIRROR);
	...
	write_cmos_sensor(0x92, GC8034Z5156CC132_FULL_STARTY);
	write_cmos_sensor(0x94, GC8034Z5156CC132_FULL_STARTX);
...
static void binning_setting(void)
{
	...
	/* Crop window */
	write_cmos_sensor(0x90, 0x01);
	write_cmos_sensor(0x92, GC8034Z5156CC132_BIN_STARTY);
	write_cmos_sensor(0x94, GC8034Z5156CC132_BIN_STARTX);
...
static void slim_video_setting(void)
{
	...
	write_cmos_sensor(0x94, 0xb0 + GC8034Z5156CC132_BIN_STARTX);
...
UINT32 GC8034Z5156CC132_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
