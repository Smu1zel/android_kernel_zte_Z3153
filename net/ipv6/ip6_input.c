...
#include <linux/tcp.h>/*zte add for output ipv6 head flag */
#include <net/tcp.h>/*zte add for output ipv6 head flag */
...
xt_socket_get6_print(struct sk_buff *skb, int direction)
{
	...
	int ulen = 0;
	struct tcphdr *th = NULL; /*zte add for output ipv6 head flag */
	...
	/*zte add for output ipv6 head flag start*/
	if (tproto == IPPROTO_TCP) {
     /*ignore checking tcp pkts correct, value of uid sometimes too large*/
		th = tcp_hdr(skb);
		pr_log_info("[IPv6]%s%d, uid=%d, Gpid:%d (%s), %pI6 :%hu -> %pI6 :%hu, F:%d%d%d%d%d%d%d%d\n",
	...
	/*zte add for output ipv6 head flag end*/
	} else {
